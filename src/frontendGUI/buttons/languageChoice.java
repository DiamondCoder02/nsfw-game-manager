...
public class languageChoice {
	...
	public static void langChoose() {
		...
		String oldLang = loadSettings.language;
		...
		String newLang = oldLang;
		if (result == JOptionPane.OK_OPTION) {
			for (int i = 0; i < buttons.length; i++) {
				if (buttons[i].isSelected()) {
					changeSettings.changeSetting("appLanguage", buttons[i].getActionCommand());
					newLang = buttons[i].getActionCommand();
				}
			}
			JOptionPane.showMessageDialog(null, "Please restart the app for the changes to take effect\n" + oldLang + " -> " + newLang , "Restart", JOptionPane.INFORMATION_MESSAGE);
		}
	}
}
