...
public class log {
	 ...
	 * @param errorLevel 0 - Info 1 - Warning 2 - Error
	 */
	public static void print(String message, int errorLevel) {
		StringBuilder fullMessage = new StringBuilder();
		switch(errorLevel) {
			case INFO:
				fullMessage.append("[").append(formatter.get().format(new Date())).append("] [Info]  - ");
				// fullMessage.append("[Info] ");
				break;
			case WARNING:
				fullMessage.append("[").append(formatter.get().format(new Date())).append("] [Warn]  - ");
				// fullMessage.append("[Warning] ");
				break;
			case ERROR:
				fullMessage.append("[").append(formatter.get().format(new Date())).append("] [Error] - ");
	...
	/**
	 * This will print a message in the default JOptionPane frame that will pop up for user
	 * @param message Message that you want to send to print.
	 */
